...
# Definir a série de dados de vazão
vazao = np.array([p10, p20, p30, p40, p50, p60, p70, p80, p90, p100])

# Definir os percentis que queremos calcular1.5
percentis = np.arange(10, 101, 10)

# Calcular os valores dos percentis
valores_percentis = np.percentile(vazao, percentis)

# Plotar os percentis
plt.figure(figsize=(10, 6))
plt.plot(percentis, valores_percentis, marker='o', linestyle='-', color='b')
for i in range(len(vazao)):
    plt.text(valores_percentis[i], vazao[i], f'{vazao[i]:.1f}', fontsize=9, ha='left')
plt.title('IP Forecast - Eq.Darcy')
plt.xlabel('Percentis (%)')
plt.ylabel('Vazão')
plt.grid(True)
#plt.xticks(np.arange(10, 101, 10))
#plt.yticks(np.arange(min(valores_percentis), max(valores_percentis)+1, 5))
plt.show()
